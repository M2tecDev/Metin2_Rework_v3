...
| `db: QUERY_LOGIN: SQL error [message]` | MariaDB query failed — bad credentials, missing table, or DB not running | Check `db/db.conf` credentials; verify the `account` table exists; check MariaDB status | Login succeeds; no SQL error in db/syserr |
